...
- [x] Projektstruktur anlegen:
  ...
  tests/
  scripts/        # Einstiegspunkte (train.py, measure.py, serve.py, check_env.py)
  models/         # ausgeliefertes Netz (best.pt)
  config.py
  ```
...
- [x] Code aufgeräumt: veraltete Querverweise in den Docstrings entfernt,
      ungenutzter Import weg, Checkpoint-/Log-Pfade werden gegen die
      Projektwurzel aufgelöst (`config.project_path`) statt gegen das
      Arbeitsverzeichnis – der Aufrufort der Skripte ist damit egal.
- [x] Trainiertes Modell als `models/best.pt` (9 MB) ins Repo aufgenommen
      (Gitignore-Ausnahme), damit das Frontend ohne eigenes Training läuft.
- [x] README gestrafft; dieser Plan liegt jetzt unter `docs/plan.md`.
- [x] Deliverable: Gut dokumentiertes Projekt, das Aufbau der KI zeigt und
      erklärt. Fertig trainiertes Modell, das menschliche Spieler zuverlässig
      schlägt (im Selbsttest ab Stufe „Mittel" nicht mehr zu gewinnen).
...
- [x] Phase 3 – Frontend (Meilenstein 3)
- [x] Schritt 4 – Cleanup (Projekt abgeschlossen)
